...
trait UsesGradingSchema
{
    ...
    private function migrateGradingTables(): void
    {
        ...
        // ── certificates ─────────────────────────────────────────────────────
        // Mirrors database/migrations/tenant/02_assessment_and_exams/
        // ..._create_certificates_table.php. Needed because publishing a
        // result (POST /result/publish) issues a certificate as part of
        // the same request.
        Schema::create('certificates', function (Blueprint $table): void {
            ...
            $table->foreign('assessment_result_id')
                ->references('result_id')->on('assessment_results')
                ->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('exam_id')
                ->references('exam_id')->on('exams')
                ->onUpdate('cascade')->onDelete('restrict');
